...
function getVelocity(rect, cir, impulse, vCollisionNorm) {
    rect.vx -= (impulse * cir.mass * vCollisionNorm.x);
    rect.vy -= (impulse * cir.mass * vCollisionNorm.y);
    cir.vx += (impulse * rect.mass * vCollisionNorm.x);
    cir.vy += (impulse * rect.mass * vCollisionNorm.y);

    return {x: rect.vx, y: rect.vy}
}
    ...
    for (let i = 0; i < gameObject.length; i++) {
        ...
        for (let j = i + 1; j < gameObject.length; j++) {
            ...
            if (rectCircleIntersect(obj1.x, obj1.y, obj1.width, obj1.height, obj2.x, obj2.y, obj2.radius)) {
                ...
                let newV = getVelocity(obj1, obj2, impulse, vCollisionNorm)

                addInfo(vCollision, vCollisionNorm, newV)
...
function addInfo(vCollision, vCollisionNorm, newV) {
    ...
    info.innerText = `
        ...
        New Velocity of rectangle:
        x: ${newV.x.toFixed(2)}, y: ${newV.y.toFixed(2)}
    `;
}